#!/usr/bin/env bash
# log-run.sh — Appends one JSON line to aisdlc/meta/run-log.jsonl after every session.
# Called explicitly by each skill in its "Final step" section (NOT as a Stop hook).
#
# Usage: bash aisdlc/hooks/log-run.sh "<skill>" "<task_summary>" "<outcome>"
...
# Output directory for run-log.jsonl — sibling of this hooks/ dir (i.e. aisdlc/meta/)
META_DIR="$(dirname "$0")/../meta"

# Read session metrics captured by the status line (capture-session-metrics.sh).
# Uses the same user-scoped temp file written by capture-session-metrics.sh.
SESSION_METRICS_FILE="/tmp/.aisdlc-session-metrics-${USER:-session}.json"
