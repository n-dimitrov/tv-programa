...
# dated JSON snapshot in aisdlc/meta/metrics-snapshots/.
#
...
#
# Usage:
#   bash aisdlc/hooks/compute-metrics.sh [LOG_FILE] [DATE_RANGE] [OUTPUT_FILE]
#
#   LOG_FILE:    Path to run-log.jsonl (default: aisdlc/meta/run-log.jsonl)
#   DATE_RANGE:  YYYY-MM-DD:YYYY-MM-DD filter (default: last 7 days)
#   OUTPUT_FILE: Output JSON path (default: aisdlc/meta/metrics-snapshots/YYYY-MM-DD.json)
#
# Dependencies: jq (JSON parsing), bc (arithmetic), bash 3.2+
# Follows conventions from aisdlc/hooks/log-run.sh (set -euo pipefail, dirname paths).
...
echo "  Next: open aisdlc/knowledge/metrics/assessment/quarterly-template.md"
echo "────────────────────────────────────────────────────────────"
