...
**Data source:** `aisdlc/meta/run-log.jsonl` — automated, computed weekly.
...
The fraction of AI sessions where the AI demonstrably loaded and applied the correct context — aisdlc/INDEX.md, relevant ADRs, patterns — as evidenced by session outcomes and task summaries that reference documented decisions.
...
**Data source:** `aisdlc/meta/run-log.jsonl` (automated) + quarterly session audit (manual).

**Interpretation:** A high rate means the documentation structure is working — the AI can find and apply the right context without coaching. A low rate means INDEX.md load order, ADR coverage, or pattern documentation needs attention.
...
Any Claude Code invocation that is logged to `aisdlc/meta/run-log.jsonl` with a named skill — i.e., where `skill != "unknown"`. Non-agentic sessions are ad-hoc prompts that bypassed the skill system.
...
A sub-metric whose value is computed directly from `aisdlc/meta/run-log.jsonl` by `aisdlc/hooks/compute-metrics.sh` without human input. Updated weekly.

### Manual Metric

A sub-metric that requires human observation and entry in `aisdlc/knowledge/metrics/assessment/quarterly-template.md`. Updated quarterly.
