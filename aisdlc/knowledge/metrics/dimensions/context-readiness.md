...
**Definition**: Percentage of active skills, components, and architectural decisions that have a corresponding documentation artifact — a `SKILL.md`, a component doc in `aisdlc/knowledge/components/`, or an ADR.
...
**Collection**: Hybrid, quarterly — `ls .claude/skills/*/SKILL.md | wc -l` for skill docs; `ls aisdlc/knowledge/architecture/adr/ADR-*.md | wc -l` for ADRs; manual component inventory.
...
**Definition**: Percentage of files in the project's knowledge directories (`aisdlc/knowledge/`, `aisdlc/INDEX.md`, `.claude/skills/`) that are stored in structured, machine-parseable formats — Markdown (`.md`), JSON (`.json`), or JSONL (`.jsonl`) — rather than binary, image, or unstructured formats.
...
**Data source**: Automated — file extension analysis across `aisdlc/knowledge/` and `.claude/skills/`. Computed by `aisdlc/hooks/compute-metrics.sh`.
...
**Definition**: The fraction of AI sessions where the AI demonstrably applied documented context — aisdlc/INDEX.md, ADRs, patterns — correctly without the human needing to re-provide already-documented information. See [glossary.md](../glossary.md#context-retrieval-success-rate) for the full definition.
...
**Data source**: `aisdlc/meta/run-log.jsonl` (automated signal) + quarterly manual session audit.
