...
Complete this template once per quarter. Automated metrics are pulled from `aisdlc/meta/metrics-snapshots/` (run `bash aisdlc/hooks/compute-metrics.sh` first). Manual metrics require observation and counting described in each section.
...
Run `bash aisdlc/hooks/compute-metrics.sh` before filling this section. Copy values from the output JSON.
...
ADR files present:                          _____   (run: ls aisdlc/knowledge/architecture/adr/ADR-*.md | wc -l)
```
...
```
Run: find aisdlc/knowledge .claude/skills aisdlc/INDEX.md -type f | wc -l     Total: _____
Run: find aisdlc/knowledge .claude/skills -name "*.md" -o -name "*.json" -o -name "*.jsonl" -o -name "*.sh" | wc -l    Structured: _____
...
**Manual audit** — Review 10 recent sessions (or all if <10). For each, did the AI correctly apply documented context (aisdlc/INDEX.md, ADRs, patterns) without you re-providing it?
...
AI sessions reviewed (skill contains "reviewer"):  _____
  (run: grep '"reviewer"' aisdlc/meta/run-log.jsonl | wc -l)
