...
- **`/onboard`** — Generates `aisdlc/INDEX.md` and a `aisdlc/` structure for your project. Run this once at the start.
...
Download and extract `ai-sdlc-starter.zip` into your project root. This gives you the skills and `aisdlc/` structure. `aisdlc/INDEX.md` will be generated by `/onboard` in Step 2.
...
The zip contains:
- `.claude/skills/` — all lifecycle skills (onboard, architect, planner, builder, reviewer, promote, skill-creator)
- `aisdlc/` — knowledge templates, memory buffers, hooks, and the metrics framework
...
You now own all of these files — they're yours to modify. The framework version is recorded in `.ai-sdlc-version`.
...
- `aisdlc/INDEX.md` — project context file
...
```bash
git add aisdlc/ .claude/skills/
git commit -m "Add ai-sdlc framework vX.Y.Z"
```

`aisdlc/INDEX.md` is generated by `/onboard` (Step 2), so it's included in this commit alongside the framework files.

Now your team has shared context. Each developer's root `CLAUDE.md` should reference `aisdlc/INDEX.md` in its load order so Claude Code picks up the project context automatically. Each developer uses the builder skill for their role.
...
**Week 1-2**: `/onboard` gives you a foundation. `aisdlc/INDEX.md` is accurate. Patterns docs may be thin.
...
**Month 3+**: The docs are a living record. New team members read `aisdlc/INDEX.md` + the ADR index to orient themselves. Each builder role has its own pattern library. Claude is a consistent collaborator across every part of the stack.
...
1. Root `CLAUDE.md` covers the whole repo: global stack, team conventions, navigation (user-owned).
2. Each service/domain gets its own `aisdlc/INDEX.md` (generated by `/onboard`).
