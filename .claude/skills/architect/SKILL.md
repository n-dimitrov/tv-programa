...
2. `docs/architecture/adr/README.md` — read the ADR index to understand decisions already made (load only this index, not every ADR)
3. Any specific ADRs that are clearly relevant to this decision (the index will tell you which)
4. `docs/architecture/overview.md` if the decision touches system topology

If any of these files don't exist yet, run `/onboard` first — making architectural decisions without context is how you get decisions that contradict what's already built.
...
**Determine the next ADR number**: Read `docs/architecture/adr/README.md` — use the next sequential number after the highest one listed.

**Create the file**: `docs/architecture/adr/ADR-XXXX-short-name.md`
...
1. **Update the ADR index** (`docs/architecture/adr/README.md`): Add a row to the table with the new ADR number, title, status (Accepted), and date.

2. **Update `docs/architecture/overview.md`**: If the decision changes the topology, data flow, or key boundaries described there, update the relevant section. If the file doesn't exist yet, don't create it here — note it for `/onboard`.

3. **Update component docs**: If the decision significantly changes how a component works or is used, update its `docs/components/[name].md`. Add a reference to the new ADR.
